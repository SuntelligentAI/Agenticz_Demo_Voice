...
export const WEB_CHAT_ENABLED_KEY = 'web_chat_enabled';
...
export async function isWebChatEnabled(db) {
  const v = await getSetting(db, WEB_CHAT_ENABLED_KEY, 'false');
  return v === 'true';
}

export async function setWebChatEnabled(
  db,
  enabled,
  updatedBy,
  reason = null,
