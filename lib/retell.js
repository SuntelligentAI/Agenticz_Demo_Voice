...
// Creates an in-browser WebRTC call. Returns { callId, accessToken } — the
// access token is handed to the Retell Web Client SDK in the browser to
// join the call.
export async function createWebCall({
  ...
  const response = await request('POST', '/v2/create-web-call', body);
  const callId = response?.call_id;
  const accessToken = response?.access_token;
  if (!callId || !accessToken) {
    throw new Error(
      `Retell create-web-call returned unexpected shape: ${JSON.stringify(response).slice(0, 300)}`,
    );
  }
...
// Chat agents — Retell's text-chat flavour of agent. If the exact REST path
// differs in your Retell workspace/version, swap the path strings here.
// Docs: https://docs.retellai.com/build/create-chat-agent
export async function createChatAgent(body) {
  if (!body || typeof body !== 'object') {
    throw new Error('createChatAgent: body is required');
  }
  return request('POST', '/create-chat-agent', body);
}

export async function updateChatAgent(agentId, fields = {}) {
  if (!agentId) throw new Error('agentId is required');
  if (!fields || typeof fields !== 'object' || !Object.keys(fields).length) {
    throw new Error('updateChatAgent: must supply at least one field');
  }
  return request(
    'PATCH',
    `/update-chat-agent/${encodeURIComponent(agentId)}`,
    fields,
  );
}

export async function getChatAgent(agentId) {
  if (!agentId) throw new Error('agentId is required');
  return request('GET', `/get-chat-agent/${encodeURIComponent(agentId)}`);
}
