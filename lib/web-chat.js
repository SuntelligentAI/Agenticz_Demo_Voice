...
// Same 15-min stage TTL as Receptionist + Web Voice Bot.
...
export async function stageDemoForUser({
  ...
  await db.execute({
    sql: `INSERT INTO web_chat_stages (
...
export async function getActiveStageForUser({
  ...
  const r = await db.execute({
    sql: `SELECT * FROM web_chat_stages
          WHERE user_id = ? AND cleared_at IS NULL AND expires_at > ?
...
export async function clearStageForUser({
  ...
  const existing = await db.execute({
    sql: `SELECT id FROM web_chat_stages
          WHERE user_id = ? AND cleared_at IS NULL AND expires_at > ?
          LIMIT 1`,
    args: [userId, now],
  });
  if (!existing.rows[0]) return { ok: true, cleared: 0 };
  await db.execute({
    sql: `UPDATE web_chat_stages
          SET cleared_at = ?
          WHERE user_id = ? AND cleared_at IS NULL AND expires_at > ?`,
...
// Used by the webhook to snapshot context into demo_calls.
