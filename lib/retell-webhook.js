...
async function ensureDemoCallRow({ call, retellCallId, receivedAt, db }) {
  ...
  // Only auto-create a row for inbound receptionist calls. Speed To Lead
  // always creates the row server-side at trigger time, so a missing row
  // there is a genuine "unknown call" case.
  if (!isReceptionistCall(call)) return null;
