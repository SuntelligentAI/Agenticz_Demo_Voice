...
import { getMostRecentActiveStage } from './receptionist.js';
import { getMostRecentActiveStage as getMostRecentActiveWebChatStage } from './web-chat.js';
...
function isReceptionistCall(call) {
  const agentId = process.env.RETELL_RECEPTIONIST_AGENT_ID || '';
  const number = process.env.RETELL_RECEPTIONIST_NUMBER || '';
  if (agentId && call?.agent_id === agentId) return true;
  if (number && call?.to_number === number) return true;
  // Retell sometimes names the direction field `direction`.
  if (number && call?.direction === 'inbound' && call?.to_number === number) {
    return true;
  }
  return false;
}

function isWebChatCall(call) {
  const agentId = process.env.RETELL_CHAT_AGENT_ID || '';
  if (agentId && call?.agent_id === agentId) return true;
  return false;
}

// Retell labels the session differently across products: voice calls use
// `call_id`, chat sessions sometimes use `chat_id`. Accept either.
function extractRetellId(call) {
  return call?.call_id || call?.chat_id || null;
}

function isChatEvent(event) {
  return (
    typeof event === 'string' &&
    (event === 'chat_started' || event === 'chat_ended' || event === 'chat_analyzed')
  );
}

async function ensureDemoCallRow({ call, retellCallId, receivedAt, db }) {
  const lookup = await db.execute({
    sql: 'SELECT id, started_at FROM demo_calls WHERE retell_call_id = ?',
    args: [retellCallId],
  });
  if (lookup.rows[0]) return lookup.rows[0];

  // Speed To Lead + Website Voice Bot both create their demo_calls row at
  // trigger time, so a missing row there is a genuine "unknown call" case.
  // Only the inbound Receptionist and the web-chat bot need auto-create.
  if (isReceptionistCall(call)) {
    return createReceptionistRow({ call, retellCallId, receivedAt, db });
  }
  if (isWebChatCall(call)) {
    return createWebChatRow({ call, retellCallId, receivedAt, db });
  }
  return null;
}

async function createReceptionistRow({ call, retellCallId, receivedAt, db }) {
  const stage = await getMostRecentActiveStage({
    db,
    clock: () => receivedAt,
  });

  const fromNumber =
    typeof call?.from_number === 'string' ? call.from_number : 'unknown';
  const userId = stage?.userId || 'system-anonymous';
  const agentName = stage?.agentName || 'Agenticz demo line';
  const companyName = stage?.companyName || 'Agenticz demo line';
  const companyDescription =
    stage?.companyDescription || 'Agenticz receptionist demo line (fallback).';
  const callPurpose =
    stage?.callPurpose ||
    'Caller reached the Agenticz demo line outside of a staged demo.';
  ...
  console.info(
    `[webhook] created receptionist demo_call ${newId} for retell_call_id=${retellCallId} user=${userId}`,
  );
  return { id: newId, started_at: null };
}

async function createWebChatRow({ call, retellCallId, receivedAt, db }) {
  const stage = await getMostRecentActiveWebChatStage({
    db,
    clock: () => receivedAt,
  });

  const userId = stage?.userId || 'system-anonymous';
  const agentName = stage?.agentName || 'Agenticz demo chat';
  const companyName = stage?.companyName || 'Agenticz demo chat';
  const companyDescription =
    stage?.companyDescription || 'Agenticz web-chat demo (fallback).';
  const callPurpose =
    stage?.callPurpose ||
    'Visitor opened the web chat outside of a staged demo.';
  ...
  console.info(
    `[webhook] created web_bot demo_call ${newId} for retell_id=${retellCallId} user=${userId}`,
  );
  return { id: newId, started_at: null };
}
...
export async function applyWebhookEvent({
  ...
  const retellCallId = extractRetellId(call);
  if (!retellCallId) {
    return { status: 200, body: { ok: true, ignored: 'missing_call_id' } };
  }

  const row = await ensureDemoCallRow({
    call,
    retellCallId,
    receivedAt,
    db,
  ...
  if (event === 'call_started') {
    ...
    // Text chat session started — no recording, no voice-specific metadata.
    const startedAt = asMs(call.start_timestamp) ?? receivedAt;
    await db.execute({
      sql: `UPDATE demo_calls
            SET status = 'in_progress', started_at = ?
  ...
  } else if (event === 'chat_ended') {
    // Chat sessions typically ship the transcript + analysis in a single
    // event, so this branch does what call_ended + call_analyzed do for
    // voice, minus any recording URL.
    const startTs = asMs(call.start_timestamp) ?? row.started_at ?? null;
    const endTs = asMs(call.end_timestamp) ?? receivedAt;
    const durationSeconds =
      startTs && endTs ? Math.max(0, Math.round((endTs - startTs) / 1000)) : null;
    ...
    const transcript =
      typeof call.transcript === 'string'
        ? call.transcript
        : typeof call.chat_transcript === 'string'
          ? call.chat_transcript
          : null;
    const aiSummary =
      typeof call?.chat_analysis?.chat_summary === 'string'
        ? call.chat_analysis.chat_summary
        : typeof call?.call_analysis?.call_summary === 'string'
          ? call.call_analysis.call_summary
          : null;
    const captured = extractCapturedFields({
      call_analysis: call?.chat_analysis || call?.call_analysis,
    });
    const capturedFieldsJson = captured ? JSON.stringify(captured) : null;
  ...
  } else if (event === 'chat_analyzed') {
    // Some deployments may fire this separately from chat_ended. Persist
    // the same artefacts without touching status.
    const transcript =
      typeof call.transcript === 'string'
        ? call.transcript
        : typeof call.chat_transcript === 'string'
    ...
    await db.execute({
      sql: `UPDATE demo_calls
            SET transcript = ?, ai_summary = ?, captured_fields = ?
            WHERE id = ?`,
      args: [transcript, aiSummary, capturedFieldsJson, demoCallId],
    });
  }
